...
docker exec -it $docker_id sed -i "s/'database' => 'drupal'/'database' => 'drupal'/" /var/www/html/sites/default/settings.php
docker exec -it $docker_id sed -i "s/'username' => 'admin'/'username' => 'root'/" /var/www/html/sites/default/settings.php
docker exec -it $docker_id sed -i "s/'password' => 'changeme'/'password' => '$mysql_root_pass'/" /var/www/html/sites/default/settings.php
drupal_admin_pass=$(pwgen -Bvs 12 1)
echo -e $drupal_admin_pass > drupal_admin_pass
docker cp drupal_admin_pass $docker_id:/etc/drupal_admin_pass
rm -f drupal_admin_pass
docker exec -it $docker_id /bin/bash -c 'cd /var/www/html; drush upwd admin --password=`cat /etc/drupal_admin_pass`; rm -f /etc/drupal_admin_pass';
custom_summary="\nDrupal Admin Username: admin"
custom_summary+="\nDrupal Admin Password: $drupal_admin_pass"
